...
import { Browser, closest, extend, Internationalization, isNullOrUndefined, removeClass, remove, compile } from '@syncfusion/ej2-base';
...
const Calendar = () => {
  const {user} = useSelector((state)=>state.auth);
    ...
    const calendarCollections = [
        { CalendarText: 'My Calendar', CalendarId: 1, CalendarColor: '#57A773' },
    ...
    const sendEventData = async (eventData) => {
      try {
        const response = await axios.post(`http://localhost:${PORT}/api/event/add-event`, eventData);
            ...
            <div className='overview-content'>
              <div className='left-panel'>
                <div className='overview-scheduler'>
                  <ScheduleComponent id='scheduler' cssClass='schedule-overview' ref={scheduleObj} width='100%' height='480px' currentView={currentView} group={{ resources: ['Calendars'] }} timezone='UTC'  dateHeaderTemplate={dateHeaderTemplate}>
                    <ResourcesDirective>
